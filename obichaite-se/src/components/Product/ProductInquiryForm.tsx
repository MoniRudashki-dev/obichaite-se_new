...
export const ProductInquiryForm = ({
  ...
  const fieldsClassName =
    variant === 'modal'
      ? 'min-h-0 flex-1 overflow-y-auto px-5 sm:px-7 py-5 sm:py-6 space-y-4 sm:space-y-5'
      : 'space-y-4 sm:space-y-5'

  const footerClassName =
    variant === 'modal'
      ? 'shrink-0 border-t border-brown/10 bg-white px-5 sm:px-7 py-3'
      : 'pt-5'
  ...
  return (
    <form
      ...
      noValidate
      onSubmit={onSubmit}
      className={variant === 'modal' ? 'flex h-[calc(90svh-92px)] min-h-0 flex-col' : 'flex flex-col'}
    >
